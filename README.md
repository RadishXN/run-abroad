...
├── nomad.html          数字游民专题
├── hk.html             香港工作签证专题
├── student.html        大学生出国路径专题
├── student-pathway.html 学生路径详情
├── css/
│   └── style.css       全站样式（自动适配深色模式）
└── js/
    ├── data.js         主项目路径数据
    ├── engine.js       主项目匹配引擎
    ├── app.js          主项目表单与结果渲染
...
大学生专题独立维护 14 条路径，包含日本语言学校、加拿大 Co-op、德国 Ausbildung、澳新打工度假、美国 J-1、互惠生、船员和换宿平台等。平台与项目不会被伪装成签证，学生数据也不会计入主项目的 71 条路径。

三个 JS 文件按顺序用 `<script>` 引入，**刻意不做模块化和打包** —— 这样任何人都能直接改一行数据然后刷新看效果，降低贡献门槛。学生专题使用独立的数据和规则树，避免把课程、实习、平台型路径硬塞进主引擎。
