...
function renderStudentDetail(p) {
  ...
  s$('#body').innerHTML = `
    ${block('先说清楚它是什么', `<p class="student-nature"><strong>${sEsc(nature)}</strong></p><p class="notes">${sMd(p.summary)}</p>`) }
    ${block('申请门槛', factsTable(p.requirements) + `<div class="student-branches">${branchHtml}</div>`) }
    ${block('钱要怎么准备', factsTable(p.costs))}
    ...
    ${p.steps?.length ? block('大致步骤', list(p.steps, true)) : ''}
    ${p.documents?.length ? block('材料清单', list(p.documents)) : ''}
    ${p.followOn ? block('之后怎么办', `<p class="notes">${sMd(p.followOn)}</p>`) : ''}
    ${p.pitfalls?.length ? block('容易踩的坑', list(p.pitfalls), 'd-warn') : ''}
    ${p.faq?.length ? block('常见问题', p.faq.map(x => `<details class="student-faq"><summary>${sEsc(x.q)}</summary><p>${sMd(x.a)}</p></details>`).join('')) : ''}
    ...
    ${block('资料说明', `<p class="notes">本条路径参考了公开路径资料的字段化整理，但参考资料不是官方来源。当前置信度：<strong>${sEsc(p.confidence)}</strong>。实际申请资格、费用、工作权和后续身份，请以链接中的官方页面为准。</p>`) }
  `;
}
